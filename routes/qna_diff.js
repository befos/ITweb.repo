...
router.get('/', function(req, res, next) {
    ...
    Forum.find({diff:query.diff},{}, {sort:{uday: -1}}, function(err, result) {
            ...
            if (result.length === 0) { //同じ_idが無い場合はDB上にデータが見つからないので0
                ...
                async.eachSeries(list, function(data2, next) {//ユーザーIDをキーにして動的にWebページの投稿者名を変更する
                    ...
                    for(i = 0 ; i < 5 ; i++){
                        var itizi;
                        itizi = i+1;
                        data.pbutton[i] = data.pbutton[i] + "diff=" + query.diff + "&page=" + itizi;
                    ...
                    switch (query.page) {
                        case '1':
                            ...
                            nextback.backurl = "/qna_diff?" + "diff=" + query.diff + "&page=1";
                            nextback.nexturl = "/qna_diff?" + "diff=" + query.diff + "&page=2";
                            nextback.backbutton = "disabled";
                            break;
                        case '2':
                            insclass.insclass2 = "active";
                            nextback.backurl = "/qna_diff?" + "diff=" + query.diff + "&page=1";
                            nextback.nexturl = "/qna_diff?" + "diff=" + query.diff + "&page=3";
                            break;
                        case '3':
                            insclass.insclass3 = "active";
                            nextback.backurl = "/qna_diff?" + "diff=" + query.diff + "&page=2";
                            nextback.nexturl = "/qna_diff?" + "diff=" + query.diff + "&page=4";
                            break;
                        case '4':
                            insclass.insclass4 = "active";
                            nextback.backurl = "/qna_diff?" + "diff=" + query.diff + "&page=3";
                            nextback.nexturl = "/qna_diff?" + "diff=" + query.diff + "&page=5";
                            break;
                        case '5':
                            insclass.insclass5 = "active";
                            nextback.backurl = "/qna_diff?" + "diff=" + query.diff + "&page=4";
                            nextback.nexturl = "/qna_diff?" + "diff=" + query.diff + "&page=5";
